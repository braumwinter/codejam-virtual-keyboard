...
let languageStorage = localStorage.getItem('languageStorage');
let letterCount = 0;
...
for (let item in buttonList) {
    ...
    if (buttonList[item].length === 2) {
        ...
        let dataKey = buttonList[item][1];

        button.setAttribute('data-key', dataKey);

        let dataAction = buttonList[item][0].toString();

        if (dataAction[0] === '&') {
            let action = dataAction.slice(1, 5);
        ...
        let dataKey = buttonList[item][4];
        button.setAttribute('data-key', dataKey);
        let mass = buttonList[item];
        mass.pop();
...
class KeyboardButton {
    ...
    Print(event) {
        let mass = event.toElement.getAttribute('data-item');
        let letters = event.toElement.getAttribute('data-item').split(',');
...
function lightCapsLock() {
    let indicat = document.getElementById('indicatorButton');
...
function changeTitle() {
    for (let buttonIndex = 0; buttonIndex < simpleButton.length; buttonIndex++) {
        let action = simpleButton[buttonIndex].getAttribute('data-item');
...
function pressCtrl(event) {
    ...
    let languageTitle = document.getElementById('language');
...
document.addEventListener('keyup', (event) => {
    ...
    for (let buttonIndex = 0; buttonIndex < allButton.length; buttonIndex++) {
        let buttonAction = allButton[buttonIndex].getAttribute('data-key');
